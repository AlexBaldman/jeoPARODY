...
11. **Question source stays canonical.** The host draws from the existing `questionService`; clients never independently choose a clue.
...
### Slice 1: proving architecture — implemented
...
- add emulator-backed Firestore Security Rules tests;
...
- outsider answer commands are ignored by host authority;
- Firestore rules restrict host-only and member-only command types;
...
- cloud release is blocked until Firestore rules/TTL policies and anonymous auth are deployed and tested on two distinct devices.
