/**
 * CategoryTracker - Migrated from Jeopardish
 * 
 * Tracks player performance by category for learning insights.
 * "Turn trivia into learning by showing where you're strong and weak."
 * 
 * @module services/CategoryTracker
 */
...
export class CategoryTracker {
  ...
  saveToStorage() {
    try {
      ...
      localStorage.setItem('jeoparody_categoryStats', JSON.stringify(data));
    } catch (error) {
      console.warn('[CategoryTracker] Unable to persist category stats.', error);
    }
  }

  loadFromStorage() {
    try {
      const data = localStorage.getItem('jeoparody_categoryStats');
      if (data) {
        const parsed = JSON.parse(data);
        this.categoryStats = new Map(Object.entries(parsed));
      }
    } catch (error) {
      console.warn('[CategoryTracker] Unable to load category stats.', error);
    }
  }
...
let categoryTrackerInstance = null;

export function getCategoryTracker() {
  if (!categoryTrackerInstance) {
    categoryTrackerInstance = new CategoryTracker();
  }
  return categoryTrackerInstance;
}
