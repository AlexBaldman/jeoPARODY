...
class QuestionDisplay extends ConnectedComponent {
  ...
  mapStateToProps(state) {
    return {
      ...
      isLoading: isLoading ? isLoading('question')(state) : false
    };
  }
   ...
   */
  render() {
    const { question, gameStatus, isLoading: loading } = this.storeState;
      ...
      ${question ? `
        <div class="question-content" data-ref="content">
          ...
          <div class="value-box" data-ref="valueBox">
            ${question.value ? `for $${String(question.value).replace(/\$/g, '')}` : ''}
          </div>
