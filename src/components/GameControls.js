...
class GameControls extends ConnectedComponent {
    ...
    mapStateToProps(state) {
        return {
            phase: selectors.game?.getPhase(state) || 'idle',
            hasQuestion: selectors.game?.hasCurrentQuestion(state) || false,
            showingAnswer: selectors.game?.isShowingAnswer(state) || false,
            currentQuestion: selectors.game?.getCurrentQuestion(state) || null,
            isLoading: selectors.ui?.isLoading(state) || false
        };
    }
    ...
    showNoAnswerWarning() {
        // Create and show a temporary warning message (with premium styling)
    ...
    handleNewQuestion() {
        ...
        // Compatibility with eventBus in main.js
        import('../utils/events.js').then(({ eventBus }) => {
    ...
    handleChatWithHost() {
        ...
        import('../services/host-system.js').then(({ getHostSystem }) => {
            const host = getHostSystem();
            if (host && host.startConversation) host.startConversation();
        }).catch(err => console.error('Failed to load host system', err));
    }
